...
export const defaults={name:'',format:'survivor',maxEntries:8,deadlineMode:'per-game',tiebreaker:'shared',doublePickWeeks:[] as number[],tiesLose:true};
...
export default function LeagueConfigFields({value,onChange,existing=false}:{value:any;onChange:(v:any)=>void;existing?:boolean}) {
 ...
 const setFormat=(format:string)=>onChange({...value,format,maxEntries:format==='survivor'?value.maxEntries||8:1,doublePickWeeks:format==='survivor'?value.doublePickWeeks||[]:[],tiesLose:format==='survivor'?value.tiesLose!==false:true});
 ...
 return <div className="space-y-4">
  ...
  <div className="grid sm:grid-cols-2 gap-4">
   ...
   {survivor&&<label>Lives per person<input className={field} type="number" min={1} max={8} required value={value.maxEntries} onChange={e=>set('maxEntries',Number(e.target.value))}/></label>}
