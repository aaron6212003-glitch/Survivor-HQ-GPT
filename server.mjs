...
export async function hqAction(client,user,input) {
 ...
 for(let attempt=0;attempt<5;attempt++) {
  ...
  if(action==='join') {
   ...
   const prior={...league.config};delete prior.buyIn;delete prior.maxEntries;delete prior.doublePickWeeks;prior.deadlineMode='per-game';
   ...
   fail('Picks lock at each game’s kickoff. Custom league deadlines are not used.');
