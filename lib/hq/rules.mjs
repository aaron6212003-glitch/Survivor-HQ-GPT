...
export function settings(input) {
 ...
 const maxEntries=input.format==='survivor'?integer(input.maxEntries,1,8,'Lives per person'):1;
 return {format:input.format,maxEntries,deadlineMode:input.deadlineMode,tiebreaker:input.format==='pickem'?input.tiebreaker:'shared',doublePickWeeks:doubles,tiesLose:input.format==='survivor' ? input.tiesLose!==false : true};
}
export function createLeague(input,user,profile) {
 const alphabet='ABCDEFGHJKLMNPQRSTUVWXYZ23456789';
 const code=[...randomBytes(6)].map(b=>alphabet[b%alphabet.length]).join('');
 const league={id:randomUUID(),code,name:label(input.name,'League name'),season:integer(input.season,2025,2030,'Season'),startWeek:integer(input.startWeek,1,18,'Starting week'),ownerId:user.id,coOwnerIds:[],config:settings(input),members:[{userId:user.id,name:profile.display_name}],entries:[],picks:[],deadlines:{},archives:[],audit:[],archived:false};
 addEntry(league,user,{name:league.config.format==='survivor'?'Life 1':profile.display_name});
 return league;
}
...
export function savePicks(league,input,user,schedules,activeWeek,now=Date.now()) {
 ...
 const isSharedManager=league.ownerId===user.id||(league.coOwnerIds||[]).includes(user.id);
 if(!entry||(!isSharedManager&&entry.userId!==user.id)) fail('You can only change entries you manage.');
...
export function viewLeague(league,user,schedules,activeWeek,selectedWeek,profileMap={},now=Date.now()) {
 const isCommissioner=league.ownerId===user.id;const isCoOwner=(league.coOwnerIds||[]).includes(user.id);const managesSharedEntries=isCommissioner||isCoOwner;
 const managedEntryIds=league.entries.filter(e=>managesSharedEntries||e.userId===user.id).map(e=>e.id);
 ...
 return {...league,members:league.members.map(m=>({...m,name:profileMap[m.userId]?.display_name||m.name})),commissioner:profileMap[league.ownerId]?.display_name||league.members.find(m=>m.userId===league.ownerId)?.name||'Commissioner',coOwners:(league.coOwnerIds||[]).map(id=>({userId:id,name:profileMap[id]?.display_name||league.members.find(m=>m.userId===id)?.name||'Co-owner'})),isCommissioner,isCoOwner,managedEntryIds,archives:league.archives.map(a=>({at:a.at,season:a.season,startWeek:a.startWeek,reason:a.reason,pickCount:a.picks.length})),entries:rows,picks,activeWeek,selectedWeek,games:(schedules[selectedWeek]||[]).map(g=>({...g,locked:gameLocked(league,selectedWeek,g,schedules[selectedWeek]||[],now)})),submissions:league.entries.map(e=>({entryId:e.id,count:league.picks.filter(p=>p.entryId===e.id&&p.week===selectedWeek).length})),me:user.id};
}
