...
export function weekDeadline(league,week,games) {
 ...
 return league.config.format!=='pickem'?Infinity:(valid.length?Math.min(...valid):Infinity);
}
...
export function savePicks(league,input,user,schedules,activeWeek,now=Date.now()) {
 ...
 if(monday) {
  ...
  if(gameLocked(league,input.week,monday,games,now)&&value!==priorPrediction) fail('The Monday Night tiebreaker is locked.');
