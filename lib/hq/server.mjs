...
function summary(row,user,meta) {
 const s=row.state;return {id:row.id,code:row.code,name:s.name,season:s.season,format:s.config.format,memberCount:1+(s.coOwnerIds||[]).length,entryCount:s.entries.filter(e=>e.userId===s.ownerId).length,owner:s.ownerId===user.id,coOwner:(s.coOwnerIds||[]).includes(user.id),activeWeek:activeWeek(s,meta),archived:s.archived};
}
...
export async function hqAction(client,user,input) {
 ...
 for(let attempt=0;attempt<5;attempt++) {
  ...
  if(['settings','deadline','reset','rollover','backfill'].includes(action)&&league.ownerId!==user.id) fail('Only the commissioner can change league settings.');
  const loaded=await leagueSchedules(client,league,meta,week);
  const opening=await nflWeek(client,league.season,league.startWeek);
  ...
  if(action==='join') {
   ...
   if((league.coOwnerIds||[]).length>=9) fail('This league already has its maximum number of co-owners.');
   league.members.push({userId:user.id,name:profile.display_name});league.coOwnerIds=[...(league.coOwnerIds||[]),user.id];
  } else if(action==='entry') {
   if(league.config.format!=='survivor') fail('Straight Pick’em gives each member one season record automatically.');
   if(started) fail('Add lives before the league’s first kickoff.');addEntry(league,user,input);
   ...
   const ownIds=new Set(league.entries.filter(e=>e.userId===user.id).map(e=>e.id));
   league.picks=league.picks.filter(p=>!ownIds.has(p.entryId)||p.week!==1);
   const historicalWeek=1; const historical=await nflWeek(client,league.season,historicalWeek); if(historical.stale||!historical.games.length) fail('The historical NFL schedule is unavailable.');
   ...
   entries.forEach((e,i)=>{e.name=`Life ${i+1}`;e.entryNumber=i+1;});
   for(let i=0;i<codes.length;i++) {
    ...
    league.picks.push({entryId:entry.id,week:historicalWeek,gameId:game.id,teamId,confidence:1,historicalResult:i===2||i===3?'loss':'win',updatedAt:new Date().toISOString()});
   }
   ...
   const prior={...league.config};delete prior.buyIn;
   if((started||league.picks.length)&&JSON.stringify(config)!==JSON.stringify(prior)) fail('Competition rules are fixed once picks are submitted or the league starts.');
