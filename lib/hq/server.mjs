...
export async function hqAction(client,user,input) {
 ...
 for(let attempt=0;attempt<5;attempt++) {
  ...
  if(action==='joinPreview') return {league:{id:league.id,name:league.name,format:league.config.format,maxEntries:league.config.maxEntries,members:league.members.map(m=>({userId:m.userId,name:m.name}))}};
  ...
  if(action==='join') {
   ...
   else {const count=league.config.format==='survivor'?integer(input.entryCount??1,1,league.config.maxEntries,'Number of lives'):1;for(let i=0;i<count;i++){const created=addEntry(league,user,{name:league.config.format==='survivor'?`Life ${i+1}`:profile.display_name});created.joinedWeek=loaded.active;}}
   ...
   const prior={...league.config};delete prior.buyIn;delete prior.maxEntries;
