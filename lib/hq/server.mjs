...
export async function hqAction(client,user,input) {
 ...
 for(let attempt=0;attempt<5;attempt++) {
  ...
  if(action==='join') {
   ...
   if(league.picks.length) fail('This league already has picks. Historical backfill can only run once.');
   if(input.confirmation!==league.name) fail('Type the league name exactly to confirm.');
   const games=loaded.schedules[league.startWeek]||[]; const codes=['JAX','JAX','LAC','LAC','DET','LV','PHI','PIT'];
   for(let i=0;i<codes.length;i++) {
    const entry=league.entries.filter(e=>e.userId===user.id).sort((a,b)=>a.entryNumber-b.entryNumber)[i];
    ...
    league.picks.push({entryId:entry.id,week:league.startWeek,gameId:game.id,teamId:codes[i],confidence:1,updatedAt:new Date().toISOString()});
   }
