...
export async function hqAction(client,user,input) {
 ...
 for(let attempt=0;attempt<5;attempt++) {
  ...
  if(action==='joinPreview') return {league:{id:league.id,name:league.name,format:league.config.format,members:league.members.map(m=>({userId:m.userId,name:m.name}))}};
  ...
  if(action==='join') {
   ...
   else {const created=addEntry(league,user,{name:league.config.format==='survivor'?'Life 1':profile.display_name});created.joinedWeek=loaded.active;}
